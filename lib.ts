...
export async function createSandbox(): Promise<Sandbox> {
  const sbx = await Sandbox.create(TEMPLATE_NAME, {
    ...
    envs: {
      ...
      TI_FS_FILE_SYSTEM_NAME: process.env.TI_FS_FILE_SYSTEM_NAME ?? '',
  ...
  try {
    ...
    try {
      await run(
        sbx,
        'mount',
        `ti fs mount-file-system --mount-path ${MOUNT_PATH} --remote-path ${REMOTE_PATH} --ready-timeout 60s`
      )
    } catch (err) {
      throw new Error(
        `ti fs mount failed inside the sandbox — check E2B network egress to the fs data plane and the TI_FS_* values in .env. Cause: ${err}`
      )
    }
    return sbx
  } catch (err) {
